/*
* TelerikReporting v9.0.15.225 (http://www.telerik.com/products/reporting.aspx)
...
(function(trv, $, window, document, undefined) {
    ...
    function ReportViewerController(options) {
        ...
        function exportReport(format, deviceInfo, isChecker,docsId) {
            throwIfNoReport();
            onExportStarted();
            ...
            exportAsync(format, deviceInfo).then(function (info) {
                ...
                if (isChecker === "isSave") {
                    ...
                    $.post(rootUrl + 'Editor/SaveTblDocsFile', { docsId: docsId, hostUrl: hostUrl, fileUrl: url }, function (data) {
        ...
        $.extend(controller, {
            ...
            exportReport: function(format, deviceInfo,isChecker,docsId) {
                return exportReport(format, deviceInfo, isChecker,docsId);
            },
...
(function(trv, $, window, document, undefined) {
    ...
    function CommandSet(options) {
        ...
        return {
            ...
            "export": new command(function(format,isSave,docsId) {
                if (format) {
                    controller.exportReport(format,null,isSave,docsId);
                }
            }),
...
(function(trv, $, window, document, undefined) {
    ...
    function ReportViewer(dom, options) {
        ...
        function eventBinder(eventName, additionalArguments, eventHandler, bind) {
            if (typeof eventHandler == "function") {
                if (bind) {
                    $(viewer).on(eventName, additionalArguments, eventHandler);
                } else {
        ...
        function attachEvents() {
            ...
            for (var eventName in viewerEventsMapping) {
                ...
                $controller.on(controllerEventName, function($viewer, eventName) {
                    return function(e) {
                        $viewer.trigger(eventName, e);
                    };
                }($viewer, eventName));
            }
        }
        function attachEventHandlers() {
            eventBinder(trv.Events.EXPORT_BEGIN, {
                sender: viewer
            }, options.exportBegin, true);
            eventBinder(trv.Events.EXPORT_END, {
                sender: viewer
            }, options.exportEnd, true);
            eventBinder(trv.Events.PRINT_BEGIN, {
                sender: viewer
            }, options.printBegin, true);
            eventBinder(trv.Events.PRINT_END, {
                sender: viewer
            }, options.printEnd, true);
            eventBinder(trv.Events.RENDERING_BEGIN, {
                sender: viewer
            }, options.renderingBegin, true);
            eventBinder(trv.Events.RENDERING_END, {
                sender: viewer
            }, options.renderingEnd, true);
            eventBinder(trv.Events.PAGE_READY, {
                sender: viewer
            }, options.pageReady, true);
            eventBinder(trv.Events.ERROR, {
                sender: viewer
            }, options.error, true);
            eventBinder(trv.Events.UPDATE_UI, {
                sender: viewer
...
/* DO NOT MODIFY OR DELETE THIS LINE! UPGRADE WIZARD CHECKSUM EA036F08589B4E86C76A5125BCE0DF7F */