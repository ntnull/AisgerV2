//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Aisger.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class MAP_FormEE2
    {
        public MAP_FormEE2()
        {
            this.Map_FormEE2Record = new HashSet<Map_FormEE2Record>();
        }
    
        public long Id { get; set; }
        public System.DateTime CreateDate { get; set; }
        public Nullable<System.DateTime> EditDate { get; set; }
        public long SecUserId { get; set; }
        public int FSCode { get; set; }
        public Nullable<double> TotalArea { get; set; }
        public Nullable<int> NumberOfStoreys { get; set; }
        public Nullable<int> DicEEStatusId { get; set; }
        public string Comments { get; set; }
        public Nullable<long> authorid { get; set; }
        public string authorlogin { get; set; }
        public bool isdeleted { get; set; }
    
        public virtual SEC_User SEC_User { get; set; }
        public virtual ICollection<Map_FormEE2Record> Map_FormEE2Record { get; set; }
    }
}
