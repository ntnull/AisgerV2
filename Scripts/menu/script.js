(function($) { 
	...
	if($('.top-nav').length > 0){
		...
		/* ================ Top navigation menu. ================ */
			...
			var clasname = "";
			if (selectMenuTitle != "") {
				var menuId = $('#' + selectMenuTitle);

				if (menuId != null) {
					//                menuId.parent().parent().parent().addClass("selected");

					var ddd = $('#RstReestr');

					var d = $('.' + selectMenuTitle);
					console.log("d=", d);

					clasname = menuId.parent().parent().parent().attr('Id');
					if (clasname == undefined)
						clasname = $('.' + selectMenuTitle).parent().parent().parent().attr('Id');

					//                $("#" + clasname).remove("in");
					//	        $("#" + clasname).addClass("selected");

					/* var idcollapse = '#' + menuId.parentElement.parentElement.parentElement.id;
					$(idcollapse).addClass("selected");*/
				}
			}

			console.log("clasname=", clasname);
			...
			items.each(function () {
				...
				if (clasname!=undefined &&  $(this).attr('id') == clasname) {
					console.log("menu=", $(this).attr('id') + "-:-" + clasname);
